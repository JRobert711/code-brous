// src/components/Login.tsx
import React, { useState } from 'react';
import { authService, tokenUtils, Usuario } from '../services/api';
import { useNavigate } from 'react-router-dom';
import ReCAPTCHA from "react-google-recaptcha";

const Login: React.FC = () => {
  const [formData, setFormData] = useState({
    dui: '',
    password: ''
  });
  const [captchaToken, setCaptchaToken] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const navigate = useNavigate();

  // Clave de reCAPTCHA para desarrollo
  const SITE_KEY = "6LfitQ0sAAAAAPf4ynSsev8KnXtM-mYoHmXBJcyP";

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    setError('');

    if (!captchaToken) {
      setError("Por favor completa el reCAPTCHA.");
      setLoading(false);
      return;
    }

    try {
      // Enviar credenciales con reCAPTCHA
      const response = await authService.login({
        ...formData,
        captcha_token: captchaToken
      });

      // Guardar token y usuario
      tokenUtils.setToken(response.data.access_token);
      tokenUtils.setUsuario(response.data.usuario);

      // Redirigir al dashboard
      navigate('/dashboard');

    } catch (error: any) {
      console.error('Error de login:', error);
      setError(error.response?.data?.detail || 'Error al iniciar sesión');
    } finally {
      setLoading(false);
    }
  };

  // Función para acceso rápido en desarrollo
  const usarTokenPrueba = (token: string, sector: string, nombres: string) => {
    tokenUtils.setToken(token);

    const usuarioPrueba: Usuario = {
      id: parseInt(token.replace('test_token_', '')),
      dui: "00000000-0",
      nombres: nombres,
      apellidos: "de Prueba",
      email: "test@email.com",
      telefono: "+503 0000-0000",
      sector: sector,
      nivel_acceso: sector === 'admin' ? 7 :
                    sector === 'medico' ? 6 :
                    sector === 'judicial' ? 5 :
                    sector === 'educativo' ? 4 :
                    sector === 'laboral' ? 3 :
                    sector === 'servicios_sociales' ? 2 : 1,
      created_at: new Date().toISOString()
    };
    
    tokenUtils.setUsuario(usuarioPrueba);
    navigate('/dashboard');
  };

  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 flex items-center justify-center p-4">
      <div className="bg-gray-800/50 border border-gray-700 rounded-2xl p-8 w-full max-w-md">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-white mb-2">🏛 Sistema Nacional</h1>
          <p className="text-gray-400">Iniciar Sesión</p>
        </div>

        {error && (
          <div className="bg-red-500/20 border border-red-500/50 text-red-400 p-3 rounded-lg mb-4">
            {error}
          </div>
        )}

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">
              DUI
            </label>
            <input
              type="text"
              value={formData.dui}
              onChange={(e) => setFormData({ ...formData, dui: e.target.value })}
              className="w-full p-3 rounded-lg bg-gray-700 border border-gray-600 text-white placeholder-gray-400 focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
              placeholder="00000000-0"
              required
              pattern="\d{8}-\d"
              title="Formato: 12345678-9"
            />
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">
              Contraseña
            </label>
            <input
              type="password"
              value={formData.password}
              onChange={(e) => setFormData({ ...formData, password: e.target.value })}
              className="w-full p-3 rounded-lg bg-gray-700 border border-gray-600 text-white placeholder-gray-400 focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
              placeholder="••••••••"
              required
            />
            <p className="text-xs text-gray-400 mt-1">
              Usa "password123" para usuarios de prueba
            </p>
          </div>

          {/* reCAPTCHA */}
          <div className="flex justify-center">
            <ReCAPTCHA
              sitekey={SITE_KEY}
              onChange={(token: string | null) => setCaptchaToken(token)}
              theme="dark"
            />
          </div>

          <button
            type="submit"
            disabled={loading || !captchaToken}
            className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-gray-600 disabled:cursor-not-allowed text-white py-3 rounded-lg font-semibold transition-colors"
          >
            {loading ? 'Verificando...' : 'Iniciar Sesión'}
          </button>
        </form>

        {/* Accesos rápidos para desarrollo */}
        <div className="mt-8 pt-6 border-t border-gray-700">
          <h3 className="text-sm font-medium text-gray-300 mb-3">Acceso Rápido (Desarrollo)</h3>
          <div className="grid grid-cols-1 gap-2">
            <button 
              onClick={() => usarTokenPrueba('test_token_5', 'admin', 'Administrador')}
              className="w-full bg-green-600 hover:bg-green-700 text-white py-2 px-4 rounded-lg text-sm transition-colors flex items-center justify-center"
            >
              <span className="mr-2">🔑</span>
              Admin (Acceso Total)
            </button>
            <button 
              onClick={() => usarTokenPrueba('test_token_1', 'ciudadano', 'Ciudadano')}
              className="w-full bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 rounded-lg text-sm transition-colors flex items-center justify-center"
            >
              <span className="mr-2">👤</span>
              Ciudadano (Básico)
            </button>
            <button 
              onClick={() => usarTokenPrueba('test_token_2', 'medico', 'Médico')}
              className="w-full bg-purple-600 hover:bg-purple-700 text-white py-2 px-4 rounded-lg text-sm transition-colors flex items-center justify-center"
            >
              <span className="mr-2">🏥</span>
              Médico (Salud)
            </button>
            <button 
              onClick={() => usarTokenPrueba('test_token_4', 'educativo', 'Educador')}
              className="w-full bg-yellow-600 hover:bg-yellow-700 text-white py-2 px-4 rounded-lg text-sm transition-colors flex items-center justify-center"
            >
              <span className="mr-2">🎓</span>
              Educativo
            </button>
          </div>
        </div>

        <div className="mt-6 text-center">
          <p className="text-xs text-gray-500">
            Sistema de Identidad Nacional v1.0<br />
            <span className="text-gray-600">Desarrollo | reCAPTCHA activo</span>
          </p>
        </div>
      </div>
    </div>
  );
};

export default Login;