...
const Login: React.FC = () => {
  ...
  const SITE_KEY = "6LfitQ0sAAAAAPf4ynSsev8KnXtM-mYoHmXBJcyP"; // <-- Tu site key
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Enviar captcha_token junto con dui y password
      ...
      <div className="bg-gray-800/50 border border-gray-700 rounded-2xl p-8 w-full max-w-md">
        ...
        <div className="mt-8 pt-6 border-t border-gray-700">
          ...
          <div className="space-y-2">
            <button onClick={() => usarTokenPrueba('test_token_999', 'admin', 'Administrador')}
              className="w-full bg-green-600 hover:bg-green-700 text-white py-2 rounded-lg text-sm transition-colors">
              🔑 Admin (Acceso Total)
            </button>
            <button onClick={() => usarTokenPrueba('test_token_1', 'ciudadano', 'Ciudadano')}
            ...
            <button onClick={() => usarTokenPrueba('test_token_2', 'medico', 'Médico')}
              className="w-full bg-purple-600 hover:bg-purple-700 text-white py-2 rounded-lg text-sm transition-colors">
              🏥 Médico (Acceso Salud)
            </button>
            <button onClick={() => usarTokenPrueba('test_token_4', 'educativo', 'Educador')}
